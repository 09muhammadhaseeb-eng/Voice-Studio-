...
def openai_run(client, user_prompt: str, system: str, max_tokens: int = 1500) -> str:
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown(
            ...
            "margin-bottom:.5rem'>🔑 OpenAI Settings</div>",
            unsafe_allow_html=True
        )
        key = st.text_input(
            "OpenAI API Key",
        ...
        st.markdown(
            ...
            "Powered by GPT-4o, gTTS & Google Translate"
            "</div>",
            unsafe_allow_html=True
        )
...
def tab_tts():
    ...
    with col_left:
        ...
        if enhance_btn and text_input.strip():
            ...
            if not client:
                st.warning("⚠️ Add your OpenAI API Key in the sidebar to use AI features.")
                ...
                with st.spinner(f"✨ {enhance_mode}…"):
                    try:
                        enhanced = openai_run(client, text_input, prompts[enhance_mode])
                ...
                if st.button(f"▶ Apply: {post_act}", use_container_width=True):
                    if post_act == "Translate to English" and not client:
                        ...
                        with st.spinner("🤖 Processing…"):
                            result = openai_run(client, transcript, sys_map.get(post_act, ""))
                        st.markdown("**Result:**")
                        st.write(result)
                        if speak_back:
...
def tab_ai_script():
    ...
    client = get_openai_client()
    if not client:
        st.warning("⚠️ Add your OpenAI API Key in the sidebar to unlock this tab.")
        return
...
def main():
    ...
    st.markdown(
        ...
        "Professional voice synthesis · Speech recognition · Translation · AI scriptwriting (GPT-4o)"
        "</p>",
        unsafe_allow_html=True
    )
